...
typedef struct FCB //目录设计，画结构，指针
{
...
typedef struct USEROPEN //用户的顶层接口
{
  ...
  char topenfle;    // 表示该用户打开表项是否为空，若值为 0，表示为空，否则表示已被某打开文件占据
  char dir[DIRLEN]; // 打开文件的绝对路径名，这样方便快速检查出指定文件是否已经打开
} useropen;

typedef struct BLOCK0 //fcb放在那里
{
  unsigned short root;
...
int main()
{
  ...
  {
    if (!strcmp(command, "ls")) //之所以要加!是因为加入strcmp()函数假如相等会返回0
    {
      my_ls();
    }
    ...
    {
      scanf("%s", command);
      if (rewrite_dir(command)) //util
        my_mkdir(command);      //create
    }
    ...
    {
      scanf("%s", command);
      if (!rewrite_dir(command)) //util，将路径规范化并检查
        continue;
      fd = my_open(command); //basics

      if (0 <= fd && fd < MAXOPENFILE)
      {
        if (!openfilelist[fd].open_fcb.attribute) //attribute == 1时 if为0，意思就是当attribute为数据时，跳到else中，为目录时，留在if中
        {
          my_close(fd);
...
//接口设计 mount会对my_xx进行一些指针操作，写下来,以下都是mount对其进行的操作
// 打开文件
...
int spiltDir(char dirs[DIRLEN][DIRLEN], char *filename)  //filename的指针指向的是fcb块中的数据
{

  int bg = 0;
  int ed = strlen(filename);//指向的是文件名的地址
  if (filename[0] == '/')
    ++bg;//bg =1
  if (filename[ed - 1] == '/')
    --ed;   //最后一个字符是/时，ed-1
...
int my_open(char *filename)
{
    ...
    //创建了两个变量，一个是dirs,一个是count，而spiltdir这个函数返回的是
...
void my_save(int fd)
{
  ...
  if (file->fcbstate)
    fat_write(file->dirno, (unsigned char *)&file->open_fcb, file->diroff, sizeof(fcb)); //为接口提供信息，
  file->fcbstate = 0;
  return;
}
